...
class PagesController extends Controller
{
 public function __construct() {
    ...
    {   
        $validator = Validator::make(Input::all(), Page::$rule_add_edit);
        if ($validator->passes()) {
            $files = Input::get('files');
            $image_name = $files[0]['path'];
            $tmp_path = public_path("assets/images/pages/single/tmp/".$image_name);
            $prm_path = public_path("assets/images/pages/single/prm/".$image_name);
            File::move($tmp_path, $prm_path);
    ...
    public function getEdit($id = null)
    {
        if (isset($id)) {
            ...
            if (isset($page)) {
                ...
                $data['page_image'] = $page->image_src;
    ...
    public function getRemove($id = null)
    {
        ...
        if (isset($id)) {
            if ($id != 1) {
                $page = Page::find($id);
                if (isset($page)) {
                    $prm_path = public_path("assets/images/pages/single/prm/".$page->image_src);
    ...
    {
        // Set layout
        if (!isset($param2) && isset($param1)) { //LINK
            ...
            if (isset($page)) {
                //PAGE FOUND
                $menus = Menu::where('param_two',$param1)->first();
                $title = isset($menus->title)?$menus->title:$page->title;
            ...
            $page = Page::where('status', 1)->where('param_one', $param2)->first();
            if (isset($page)) {
                //PAGE FOUND
                $title = isset($menus->title)?$menus->title:$page->title;
                $page_content  = $page->section_content;
...
{
    error_reporting(E_ALL | E_STRICT);
    $destinationPath = public_path("assets/images/pages/single/tmp/");
    $savePath = "/assets/images/pages/single/tmp/";
    // Check if directory is made for this company if not then create a new directory
    if (!file_exists($destinationPath)) {
        mkdir($destinationPath, 0777, true);
    }
    ...
    $files = Input::file('files');
    $fileName = str_random(12).time().'.jpg';
...
public function postUploadSlider()
{
    ...
    {
        $all_sliders = Page::PrepareSlidersForIndex(Page::where('id',1)->get());
